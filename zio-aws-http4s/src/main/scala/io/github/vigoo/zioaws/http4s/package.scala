...
import io.github.vigoo.zioaws.core.{BuilderHelper, httpclient}
...
import io.github.vigoo.zioaws.http4s.tryDefaultSslContext
    ...
    object descriptors {
      ...
      val http4sChannelOptions: ConfigDescriptor[ChannelOptions] =
        channelOptions.transform(
          opts => ChannelOptions(opts.options.map(opt => OptionValue(opt.key, opt.value))),
          opts => httpclient.ChannelOptions(opts.options.map(opt => httpclient.OptionValue(opt.key, opt.value)))
        )
      ...
      val blazeClientConfig: ConfigDescriptor[BlazeClientConfig] = (
        duration("responseHeaderTimeout").default(Duration.Inf) ?? "Timeout for receiving the header part of the response" |@|
          ...
          nested("channelOptions")(http4sChannelOptions).default(ChannelOptions(Vector(OptionValue[java.lang.Boolean](java.net.StandardSocketOptions.TCP_NODELAY, true)))) ?? "Collection of socket options"
