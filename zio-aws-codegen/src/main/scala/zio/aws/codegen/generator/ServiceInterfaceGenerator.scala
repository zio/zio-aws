...
trait ServiceInterfaceGenerator {
  ...
  private def serviceType
    ...
    for {
      ...
      name <- ZIO
        .attempt(ctx.namingStrategy.getServiceName)
        .mapError(UnknownError)
    } yield ScalaType(ctx.pkg, name)
  ...
  private def opRequestType(
    ...
    for {
      ctx <- ZIO.service[AwsGeneratorContext]
      name <- ZIO
        .attempt(ctx.namingStrategy.getRequestClassName(opName))
        .mapError(UnknownError)
    } yield ScalaType(ctx.pkg / "model", name)
  ...
  private def opResponseType(
    ...
    for {
      ctx <- ZIO.service[AwsGeneratorContext]
      name <- ZIO
        .attempt(ctx.namingStrategy.getResponseClassName(opName))
        .mapError(UnknownError)
    } yield ScalaType(ctx.pkg / "model", name)
  ...
  private def findEventStreamShape(
    ...
    ZIO.serviceWithZIO[AwsGeneratorContext] { ctx =>
      ...
      ModelCollector.tryFindEventStreamShape(models, outputShape) match {
        ...
        case None =>
          ZIO.fail(CannotFindEventStreamInShape(ctx.serviceName, outputShape))
      }
    }
  }
  ...
  private def findShape(
    ...
    ZIO.serviceWithZIO[AwsGeneratorContext] { ctx =>
      val models = ctx.models
      val serviceName = ctx.serviceName
  ...
  private def generateEventStreamInputOutput(
    ...
    for {
      ...
      inEventT <- ctx.get(inEventModel.getShape).map(_.generatedType)
      ...
      outEventT <- ctx.get(outEventModel.getShape)
  ...
  private def generateEventStreamOutput(
    ...
    for {
      ...
      eventItemModel <- ctx.get(rawEventItem.getShape)
  ...
  private def generateEventStreamInput(
    ...
    for {
      ...
      rawEventItem = eventStream.shape.getMembers.asScala.values.head
      eventT <- ctx.get(rawEventItem.getShape).map(_.generatedType)
    ...
    pagination match {
      ...
      case Some(
        ...
        for {
          ...
          awsItemType <- TypeMapping.toJavaType(itemModel)
          responseModel <- ctx.get(responseType.name)
      ...
      case Some(member) =>
        findShape(member.getShape).flatMap { contentLengthShape =>
          if (ModelType.fromShape(contentLengthShape.shape) == ModelType.Long) {
            ZIO
              .serviceWithZIO[AwsGeneratorContext] { ctx =>
                ctx.logInfo(
    ...
    for {
      ...
      sdkPackage = id.subModuleName match {
        case Some(submodule) if submodule != id.name =>
          ctx.modelPkg.parent / submodule
        case _ => ctx.modelPkg.parent
      }
