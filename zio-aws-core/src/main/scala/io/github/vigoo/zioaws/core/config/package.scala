...
package object config {
  ...
  case class CommonClientConfig(extraHeaders: Map[String, List[String]],
  ...
  case class CommonAwsConfig(region: Option[Region],
  ...
  def configured(): ZLayer[HttpClient with ZConfig[CommonAwsConfig], Nothing, AwsConfig] =
    ZLayer.fromServices[HttpClient.Service, CommonAwsConfig, AwsConfig.Service] { (httpClient, commonConfig) =>
      new AwsConfig.Service {
        override def configure[Client, Builder <: AwsClientBuilder[Builder, Client]](builder: Builder): Task[Builder] = {
          ...
          Task {
            val b0 =
              builder
                .optionallyWith(commonConfig.endpointOverride)(_.endpointOverride)
            ...
            commonConfig.commonClientConfig match {
              case Some(commonClientConfig) =>
                val clientOverrideBuilderHelper: BuilderHelper[ClientOverrideConfiguration] = BuilderHelper.apply
                ...
                val overrideBuilder =
                  ClientOverrideConfiguration.builder()
                    ...
                    .optionallyWith(commonClientConfig.defaultProfileName)(_.defaultProfileName)
  ...
  object descriptors {
    ...
    val credentialsProvider: ConfigDescriptor[AwsCredentialsProvider] = {
      val defaultCredentialsProvider: ConfigDescriptor[AwsCredentialsProvider] = string.xmapEither(
        s => if (s == "default") Right(DefaultCredentialsProvider.create()) else Left("Not 'default'"),
        {
          case _: DefaultCredentialsProvider => Right("default")
          case _ => Left("Unsupported credentials provider")
        }
      )
      val anonymousCredentialsProvider: ConfigDescriptor[AwsCredentialsProvider] = string.xmapEither(
        s => if (s == "anonymous") Right(AnonymousCredentialsProvider.create()) else Left("Not 'anonymous'"),
        {
          case _: AnonymousCredentialsProvider => Right("anonymous")
          case _ => Left("Unsupported credentials provider")
        }
      )
      val staticCredentialsProvider: ConfigDescriptor[AwsCredentialsProvider] = awsCredentials.xmap(
    ...
    val retryPolicy: ConfigDescriptor[RetryPolicy] = {
      val default: ConfigDescriptor[RetryPolicy] = string.xmapEither(
        s => if (s == "default") Right(RetryPolicy.defaultRetryPolicy()) else Left("Not 'default'"),
        (p: RetryPolicy) => if (p == RetryPolicy.defaultRetryPolicy()) Right("default") else Left("Not the default retry policy")
      )
      val legacy: ConfigDescriptor[RetryPolicy] = string.xmapEither(
        s => if (s == "legacy") Right(RetryPolicy.forRetryMode(RetryMode.LEGACY)) else Left("Not 'legacy'"),
        (p: RetryPolicy) => if (p == RetryPolicy.forRetryMode(RetryMode.LEGACY)) Right("legacy") else Left("Not the legacy retry policy")
      )
      val standard: ConfigDescriptor[RetryPolicy] = string.xmapEither(
        s => if (s == "standard") Right(RetryPolicy.forRetryMode(RetryMode.STANDARD)) else Left("Not 'standard'"),
        (p: RetryPolicy) => if (p == RetryPolicy.forRetryMode(RetryMode.STANDARD)) Right("standard") else Left("Not the standard retry policy")
      )
      val none: ConfigDescriptor[RetryPolicy] = string.xmapEither(
        s => if (s == "none") Right(RetryPolicy.none()) else Left("Not 'none'"),
        (p: RetryPolicy) => if (p == RetryPolicy.none()) Right("none") else Left("Not the 'none' retry policy")
      )
      val retryMode: ConfigDescriptor[RetryMode] = string.xmapEither(
        {
          ...
          case s: String => Left(s"Invalid retry mode '$s'. Use legacy, standard or default'")
        },
        {
      ...
      val backoffStrategy: ConfigDescriptor[BackoffStrategy] = {
        val fullJitter: ConfigDescriptor[BackoffStrategy] =
          nested("fullJitter")(
            (duration("baseDelay") |@| duration("maxBackoffTime")) (
              (baseDelay, maxBackoffTime) => FullJitterBackoffStrategy
                .builder()
                .baseDelay(zio.duration.Duration.fromScala(baseDelay))
                .maxBackoffTime(zio.duration.Duration.fromScala(maxBackoffTime))
                .build(),
              {
                case bs: FullJitterBackoffStrategy => Some(bs.toBuilder.baseDelay.asScala, bs.toBuilder.maxBackoffTime.asScala)
                case _ => None
              }
            ))
        val equalJitter: ConfigDescriptor[BackoffStrategy] =
          nested("equalJitter")(
            (duration("baseDelay") |@| duration("maxBackoffTime")) (
              (baseDelay, maxBackoffTime) => EqualJitterBackoffStrategy
                .builder()
                .baseDelay(zio.duration.Duration.fromScala(baseDelay))
                .maxBackoffTime(zio.duration.Duration.fromScala(maxBackoffTime))
                .build(),
              {
                case bs: EqualJitterBackoffStrategy => Some(bs.toBuilder.baseDelay.asScala, bs.toBuilder.maxBackoffTime.asScala)
                case _ => None
              }
            ))
        val fixed: ConfigDescriptor[BackoffStrategy] =
          nested("fixed")(
            duration("backoff")(
              backoff => FixedDelayBackoffStrategy.create(zio.duration.Duration.fromScala(backoff)),
              {
                case bs: FixedDelayBackoffStrategy => Some(bs.computeDelayBeforeNextRetry(RetryPolicyContext.builder().build()).asScala)
                case _ => None
              }
            ))
      ...
      def retryCondition: ConfigDescriptor[RetryCondition] = {
        val default: ConfigDescriptor[RetryCondition] = string.xmapEither(
          s => if (s == "default") Right(RetryCondition.defaultRetryCondition()) else Left("Not 'default'"),
        ...
        val retryOnExceptions: ConfigDescriptor[RetryCondition] =
          (listOrSingleton("retryOnException")(string)).xmapEither(
            lst => lst.foldLeft[Either[String, List[Class[Exception]]]](Left("empty list")) {
              case (Left(msg), _) => Left(msg)
              case (Right(result), name) => Try(Class.forName(name).asInstanceOf[Class[Exception]]) match {
                case Failure(exception) => Left(exception.toString)
                case Success(value) => Right(value :: result)
              }
            }.map(classes => RetryOnExceptionsCondition.create(classes : _*)),
            _ => Left("cannot extract RetryOnExceptionsCondition") // NOTE: Cannot extract conditions without reflection
          )
        val retryOnStatusCodes: ConfigDescriptor[RetryCondition] =
          (listOrSingleton("retryOnStatusCode")(int)) (
            codes => RetryOnStatusCodeCondition.create(codes.map(int2Integer).toSet.asJava),
            _ => None // NOTE: Cannot extract conditions without reflection
          )
        val tokenBucket: ConfigDescriptor[RetryCondition] =
          nested("tokenBucket")(
            (int("bucketSize") ?? "Maximum number of tokens in the token bucket" |@|
              nested("costFunction")(
                (int("throttlingExceptionCost") ?? "Number of tokens to be removed on throttling exceptions" |@|
                  int("exceptionCost") ?? "Number of tokens to be removed on other exceptions") (
                  (throttlingCost, cost) => TokenBucketExceptionCostFunction
                    .builder()
              ...
              (size, costFn) => TokenBucketRetryCondition
                .builder()
      ...
      val custom: ConfigDescriptor[RetryPolicy] =
        (nested("mode")(retryMode) ?? "Retry mode" |@|
          ...
          boolean("additionalRetryConditionsAllowed") ?? "Allows additional retry conditions" |@|
          nested("backoffStrategy")(backoffStrategy).default(BackoffStrategy.defaultStrategy()) ?? "Backoff strategy for waiting between retries" |@|
          nested("throttlingBackoffStrategy")(backoffStrategy).default(BackoffStrategy.defaultThrottlingStrategy()) ?? "Backoff strategy for waiting after throttling error" |@|
          nested("retryCondition")(retryCondition) ?? "Condition deciding which request should be retried" |@|
          nested("retryCapacityCondition")(retryCondition) ?? "Condition deciding throttling") (
          (mode, numRetries, additionalRetryConditionsAllowed, backoffStrategy, throttlingBackoffStrategy, retryCondition, retryCapacityCondition) =>
            RetryPolicy
              .builder(mode)
              ...
              .build(),
          p => Some((p.retryMode(), p.numRetries(), p.additionalRetryConditionsAllowed(), p.backoffStrategy(), p.throttlingBackoffStrategy(), p.retryCondition(), p.toBuilder.retryCapacityCondition()))
        )

      default <> legacy <> standard <> none <> custom
    }
    val commonClientConfig: ConfigDescriptor[CommonClientConfig] =
      (nested("extraHeaders")(rawHeaderMap) ?? "Extra headers to be sent with each request" |@|
        nested("retryPolicy")(retryPolicy).optional ?? "Retry policy" |@|
        duration("apiCallTimeout").optional ?? "Amount of time to allow the client to complete the execution of an API call" |@|
        duration("apiCallAttemptTimeout").optional ?? "Amount of time to wait for the HTTP request to complete before giving up" |@|
        string("defaultProfileName").optional ?? "Default profile name"
        ) (CommonClientConfig.apply, CommonClientConfig.unapply)

    val commonAwsConfig: ConfigDescriptor[CommonAwsConfig] =
      (nested("region")(region).optional ?? "AWS region to connect to" |@|
        nested("credentials")(credentialsProvider).default(DefaultCredentialsProvider.create()) ?? "AWS credentials provider" |@|
        uri("endpointOverride").optional ?? "Overrides the AWS service endpoint" |@|
        nested("client")(commonClientConfig).optional ?? "Common settings for AWS service clients"
        ) (CommonAwsConfig.apply, CommonAwsConfig.unapply)
  }
