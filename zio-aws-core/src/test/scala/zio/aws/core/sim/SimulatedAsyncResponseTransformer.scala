...
object SimulatedAsyncResponseTransformer {
  ...
  case class FailureSpec(
      failBeforePrepare: Option[Throwable] = None,
  ...
  def useAsyncResponseTransformer[In, Out](
      in: In,
      transformer: AsyncResponseTransformer[Out, Task[
        StreamingOutputResult[Any, Out, Byte]
      ]],
      toResult: In => Out,
  ...
  ): CompletableFuture[Task[StreamingOutputResult[Any, Out, Byte]]] = {
    val cf =
      new CompletableFuture[Task[StreamingOutputResult[Any, Out, Byte]]]()
    threadPool.submit(new Runnable {
      override def run(): Unit = {
        useAsyncResponseTransformerImpl(
          in,
          transformer,
          toResult,
          toPublisher,
  ...
  def useAsyncResponseTransformerImpl[Out, In](
      in: In,
      transformer: AsyncResponseTransformer[Out, Task[
        StreamingOutputResult[Any, Out, Byte]
      ]],
      ...
      toPublisher: In => SdkPublisher[ByteBuffer],
      failureSpec: FailureSpec,
      cf: CompletableFuture[Task[StreamingOutputResult[Any, Out, Byte]]]
  ): Unit = {
    failureSpec.failBeforePrepare match {
      ...
      case None =>
        slowDown()
        transformer.prepare().thenApply[Boolean] {
          (result: Task[StreamingOutputResult[Any, Out, Byte]]) =>
            slowDown()
